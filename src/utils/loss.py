...
class NoPeekLoss(torch.nn.modules.loss._Loss):
    def __init__(self, args, dcor_weighting: float = 0.1) -> None:
        super().__init__()
        self.dcor_weighting = dcor_weighting

        self.ce = torch.nn.CrossEntropyLoss()
        self.dcor = DistanceCorrelationLoss(args)
        if args.gpu > -1:
            self.ce.cuda(args.gpu)
            self.dcor.cuda(args.gpu)
    ...
    def forward(self, inputs, intermediates, outputs, targets):
        ...
        if self.dcor_weighting > 0.0:
            dist = self.dcor(inputs, intermediates)
            dcor_loss = self.dcor_weighting * dist
            #return cce_loss, dcor_loss
            return cce_loss + dcor_loss, dist
        else:
            return (cce_loss,)
...
class DistanceCorrelationLoss(torch.nn.modules.loss._Loss):
    ...
    def _distance_matrix(self, data):
        n = data.size(0)
        distance_matrix = torch.zeros((n, n)).cuda(self.args.gpu)
